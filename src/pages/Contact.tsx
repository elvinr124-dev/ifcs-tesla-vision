    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-[80vh] min-h-[600px] w-full flex items-center overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-5xl mx-auto px-6 md:px-12 text-center animate-fade-in-up hero-text-shadow">
          <p className="text-sm font-medium tracking-[0.2em] uppercase mb-3 text-white">{translate("Get in Touch")}</p>
          <h1 className="tesla-hero-title text-white">{translate("Contact Us")}</h1>
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          {[
            ...
            <div key={title} className="p-8 rounded-3xl border border-border bg-card shadow-lg hover:shadow-xl hover:border-accent/30 transition-all duration-300">
              ...
              {lines.map((l) => (
                <p key={l} className="text-sm text-muted-foreground">{translate(l)}</p>
              ))}
            </div>
          ))}
        </div>
