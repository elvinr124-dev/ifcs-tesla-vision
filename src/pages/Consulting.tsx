    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-[80vh] min-h-[600px] w-full flex items-center overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-5xl mx-auto px-6 md:px-12 text-center animate-fade-in-up hero-text-shadow">
          <p className="text-sm font-medium tracking-[0.2em] uppercase mb-3 text-white">{translate("Expert Guidance")}</p>
          <h1 className="tesla-hero-title text-white">{translate("Consulting")}</h1>
          <p className="tesla-hero-subtitle max-w-2xl mx-auto mt-4 text-white/90">
            {translate("Work side by side with a senior consultant to ensure you choose the right evaluation and streamline your admission.")}
          </p>
        </div>
      </section>
      ...
      <section className="py-24 px-6 md:px-12 content-bg">
        <div className="max-w-6xl mx-auto">
          <p className="text-sm font-medium tracking-[0.2em] uppercase text-accent text-center mb-3">{translate("What We Offer")}</p>
          <h2 className="text-3xl md:text-5xl font-semibold tracking-tight text-foreground text-center mb-16">
            {translate("Consulting Services")}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {services.map((s) => (
              <div key={s.title} className="p-8 rounded-3xl border border-border bg-card shadow-lg hover:shadow-xl hover:border-accent/30 transition-all duration-300">
                ...
                <h3 className="text-lg font-semibold text-foreground mb-2">{translate(s.title)}</h3>
                <p className="text-sm text-muted-foreground leading-relaxed">{translate(s.desc)}</p>
              </div>
            ))}
          </div>

          <div className="text-center mt-16">
            <Link to="/consulting/book" className="tesla-btn-primary inline-block">
